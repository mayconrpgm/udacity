...
```{r}
ggplot(aes(x = alcohol, y = quality), data = subset(redWines.outliers, alcohol.outlier == 0)) + 
  geom_point(aes(color = factor(outlier_count), size = outlier_count), position = "jitter") +  
...
```
Finalmente, vemos um mapa de calor onde peguei uma amostra de 50 vinhos com qualidade maior ou igual à 7, cada linha representa um vinho e as colunas são as variáveis correspondentes às propriedades químicas juntamente com a variável correspondente à qualidade, as variáveis foram normalizadas utilizando a técnica *min-max scaling*. As células do mapa indicam quão alto é o valor da variável dentro de sua própria distribuição, ou seja, quanto mais vermelha, mais para a direita do histograma dessa varíavel e quanto mais azul, mais à esquerda.
