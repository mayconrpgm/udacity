...
h5 <- ggplot(aes(x = fixed.acidity), data = redWines) + 
  geom_histogram(binwidth = 0.1) + 
  ggtitle("Histograma de fixed.acidity", "Todos os vinhos")

h6 <- ggplot(aes(x = fixed.acidity), data = redWines.hq) + 
  geom_histogram(binwidth = 0.1)  + 
  ggtitle("Histograma de fixed.acidity", "vinhos com nota igual ou superior à 7")

grid.arrange(h1, h2, h3, h4, h5, h6, ncol = 2)
```

Por fim, vejamos algumas características segmentando vinhos de qualidade igual ou superior à 7 e como eles se comparam aos vinhos no geral. Observamos as variáveis *alcohol*, *residual.sugar* e *fixed.acidity*. Ao que parece, o álcool influencia mais na qualidade quando em valores mais extremos tanto para cima quando para baixo, os vinhos de alta qualidade apresentam uma distribuição um pouco mais uniforme. Já com relação ao açúcar, vinhos mais doces foram considerados inferiores. A acidez parece ser menos relevante já que a amplitude dos valores é praticamente a mesma e a distribuição é mais uniforme.
...
O nível de açúcar e o teor alcoólico paracem ser relevantes ao determinar a qualidade.

### Quais outros atributos você acha que podem lhe auxiliar na investigação destes atributos de interesse?

As variáveis referentes à acidez podem ser úteis também para avaliar a qualidade.

https://www.clubedosvinhos.com.br/qualidade-de-um-vinho/
